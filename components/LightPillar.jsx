...
const LightPillar = ({
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        col -= fract(sin(dot(gl_FragCoord.xy, vec2(12.9898, 78.233)))
               * 43758.5453) / 15.0 * uNoiseIntensity;
  ...
  // Reactive prop updates (no remount needed)
  ...
  if (!webGLSupported) {
    return (
      <div
        className={`w-full h-full absolute inset-0 flex items-center justify-center bg-bg-main text-text-muted text-sm ${className}`}
        style={{ mixBlendMode }}
      >
        WebGL not supported
      </div>
